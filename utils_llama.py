...
def preprocess_features_function(examples, args, raw_datasets, tokenizer):
    ...
    if args.do_cot:
        ...
        examples[context_column] =  ['[INST] Read the given context and answer the questions Context: {}'.format(c.strip()) for c in examples[context_column]]
        examples[question_column] = [' Question: {}[/INST]'.format(q.strip()) for q in examples[question_column]]
