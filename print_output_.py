...
parser.add_argument("--results_dir", type=str, default="subject_results", help="Where to store the final model.")
...
subjects = sorted(
    [
        f.split(".pkl")[0]
        for f in os.listdir(os.path.join(args.output_dir, args.results_dir))
        if ".pkl" in f
    ]
)
...
for subject in subjects:
    with open(os.path.join(args.output_dir, args.results_dir, "{}.pkl".format(subject)), 'rb') as f:
