...
def main():
    ...
    if args.do_test_time_tuning:       
        ...
        test_time_tuning_dataloader = DataLoader(
            test_time_tuning_dataset_for_model, collate_fn=data_collator, batch_size=args.per_device_prob_batch_size
        )
    ...
    # MC-drop
    def run_prob(model, tokenizer, batch, gen_kwargs, args):
        logger.info("***** Running Prob *****")
        logger.info(f"  Batch size = {args.per_device_prob_batch_size}")
        ...
        with torch.no_grad():
            for i in range(0, args.mc_drop_num):
                ...
                probabilities =(
                    torch.sum(torch.exp(transition_scores), dim=1) / torch.sum(transition_scores != float('-inf'), dim = 1)
                )
                #import pdb; pdb.set_trace()
        ...
        return lst_generated_tokens, probabilities
    ...
    def run_majority_vote(tokenizer, arr_mc_generated_tokens, args):
        logger.info("***** Running Prod filtering *****")
    ...
    if args.do_eval:
        ...
        if args.do_test_time_tuning:
            ...
            for step, batch in enumerate(test_time_tuning_dataloader):
                ...
                # mc drop
                lst_mc_generated_tokens, probabilities = run_prob(model, tokenizer, batch, gen_kwargs, args)
                ...
                for filtered_idx in range(batch_size):
                    if probabilities >= args.filter_thres:
                        #import pdb; pdb.set_trace()
                        lst_filtered_input_ids.append(batch['input_ids'][filtered_idx])
                        lst_filtered_labels.append(torch.tensor(lst_mc_generated_tokens[0])[filtered_idx])
